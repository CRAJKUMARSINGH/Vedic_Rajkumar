...
export function useChartCalculation(
  ...
  const calculate = useCallback(async (inp: BirthInput) => {
    ...
    // 2. Ascendant — derive timezone from longitude for correct Lagna
    try {
      // IST = 5.5; derive from lon for non-IST births (lon/15 rounded to nearest 0.5h)
      const tzOffset = Math.round((inp.lon / 15) * 2) / 2;
      result.ascendant = calculateCompleteAscendant(inp.date, inp.time, inp.lat, inp.lon, tzOffset);
    } catch (e) {
      errs.ascendant = String(e);
    }

    // ── Extract accurate Moon longitude from the precision engine ──
    // This is the SINGLE SOURCE OF TRUTH for Moon position.
    // It flows into Nakshatra and Dasha to eliminate the legacy ±3° error.
    let accurateMoonLongitude: number | undefined;
    if (result.planetaryPositions?.moon) {
      const moon = result.planetaryPositions.moon;
      // Use sidereal longitude directly if available (Swiss Eph / VSOP87)
      if (moon.sidereal !== undefined && moon.sidereal !== 0) {
        accurateMoonLongitude = moon.sidereal;
      } else {
        // Reconstruct from rashi + degrees
        accurateMoonLongitude = moon.rashi * 30 + moon.degrees;
      }
    }

    // 3. Nakshatra — uses accurate Moon longitude when available
    try {
      result.nakshatra = getNakshatraInfo(inp.date, inp.time, accurateMoonLongitude);
    } catch (e) {
    ...
    // 5. Dasha — uses accurate Moon longitude when available
    try {
      result.dasha = calculateVimshottariDasha(inp.date, inp.time, 3, accurateMoonLongitude);
    } catch (e) {
      errs.dasha = String(e);
    }
