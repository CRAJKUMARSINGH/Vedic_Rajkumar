 ...
 *
 * FIX (2026-06): The original formula had a 180° error because atan2 arguments
 * were swapped relative to the Meeus standard. Correct formula is:
...
/**
 * Calculate Ascendant (Rising Sign) - High precision Meeus method
 */
export function calculateAscendantFromJD(
  jd: number,
  latitude: number,
  longitude: number
): number {
  const T = (jd - 2451545.0) / 36525.0;
  // Mean Sidereal Time at Greenwich (Meeus 12.4)
  const theta0 = ((280.46061837 + 360.98564736629 * (jd - 2451545.0) + 0.000387933 * T * T - T * T * T / 38710000) % 360 + 360) % 360;
  const lst = (theta0 + longitude) % 360;
  const RAMC = lst * Math.PI / 180;
  
  // High precision obliquity (Epsilon)
  const eps = (23.4392911 - (46.8150 * T + 0.00059 * T * T - 0.001813 * T * T * T) / 3600) * Math.PI / 180;
...
/**
 * Derive UTC offset from geographic longitude when no explicit timezone is given.
 * Approximation: lon / 15. Rounds to nearest 0.5h for IST compatibility.
 * For precise results always pass the actual UTC offset.
 */
function lonToUtcOffset(lon: number): number {
  // Round to nearest 0.5h — covers IST (5.5), IST+/-0.5, etc.
...
export function calculateCompleteAscendant(
  ...
  latitude: number,
  longitude: number,
  timezoneOffset?: number   // UTC offset in hours; auto-derived from longitude if omitted
): AscendantData {
  // Clamp coordinates to valid range
  const clampedLat = Math.max(-89.9, Math.min(89.9, latitude ?? 0));
  const clampedLon = Math.max(-179.9, Math.min(179.9, longitude ?? 0));

  // Use provided timezone or derive from longitude (critical for non-IST births)
  const tzOffset = timezoneOffset !== undefined ? timezoneOffset : lonToUtcOffset(clampedLon);
  ...
  try {
    ...
    // Convert local time to UTC using correct offset (not hardcoded IST)
    const [yr, mo, dy] = dateStr.split('-').map(Number);
    const [hr, mn] = timeStr.split(':').map(Number);
    const localMs = Date.UTC(yr, mo - 1, dy, hr ?? 12, mn ?? 0, 0);
