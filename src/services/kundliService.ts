...
// NormalizedPlanet has house field; re-export for convenience
export type { PlanetPosition } from './ephemerisService';
 ...
 * @param timezoneOffset UTC offset in hours (default 5.5 for IST). Pass actual offset for non-IST births.
 */
export function calculateKundli(
  ...
  // Calculate ascendant and houses — pass timezone so Lagna is correct
  const ascendant = calculateCompleteAscendant(date, time, latitude, longitude, timezoneOffset);
  
  // Calculate planetary positions — IST assumption baked into legacy service;
  // for precision use precisionEphemerisService which accepts timezoneOffset
  const planetaryData = calculateCompletePlanetaryPositions(date, time);

  // Assign houses using whole-sign system from corrected ascendant
  const ascRashiIndex = ascendant.ascendant.rashiIndex;
  const planetsWithHouses = planetaryData.planets.map(p => ({
    ...p,
    house: (p.rashiIndex - ascRashiIndex + 12) % 12 + 1,
  }));
  ...
  for (let i = 0; i < 12; i++) {
    const houseRashi = (ascRashiIndex + i) % 12;
    const rashiData = RASHI_DATA[houseRashi];
    const significance = HOUSE_SIGNIFICANCE[i];
    
    // Find planets in this house (whole-sign)
    const planetsInHouse = planetsWithHouses.filter(planet => planet.house === i + 1);
  ...
  // Calculate aspects and dignities using whole-sign house placements
  const aspectsData = calculatePlanetaryAspects(planetsWithHouses);
  const dignityData = calculatePlanetaryDignities(planetsWithHouses);
