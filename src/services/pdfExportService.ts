...
// ─────────────────────────────────────────────────────────────────────────────
// Transit Analysis PDF with bilingual narrative
// Called from TransitAnalysisPage to produce consultation-ready reports.
// ─────────────────────────────────────────────────────────────────────────────

export interface TransitNarrativePDFData {
  birthDate: string;
  birthTime: string;
  birthLocation: string;
  latitude?: string;
  ...
  results: TransitResult[];
  overallScore: number;
  /** Full English narrative paragraphs */
  narrativeEn: string;
  /** Full Hindi narrative paragraphs */
  narrativeHi: string;
  /** Ayanamsa used */
  ayanamsa?: string;
  /** Ephemeris engine ('SwissEph' | 'fallback') */
  ephemerisMode?: string;
}

/**
 * Export a transit analysis PDF that includes:
 *  1. The standard transit table (reuses exportTransitToPDF layout)
 *  2. Bilingual narrative section (English + Hindi) appended after the table
 *  3. Metadata footer: ayanamsa, ephemeris mode, timestamp
 */
export function exportTransitWithNarrative(
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(
    isHi ? 'गोचर विश्लेषण रिपोर्ट' : 'Transit Analysis Report',
  ...
  doc.text(
    isHi
      ? `${data.birthDate} | ${data.birthTime} | ${data.birthLocation} | चंद्र: ${data.moonRashi}`
      : `${data.birthDate} | ${data.birthTime} | ${data.birthLocation} | Moon: ${data.moonRashi}`,
    pageWidth / 2, 20, { align: 'center' },
  );
  doc.text(
    isHi ? `गोचर तिथि: ${data.transitDate}` : `Transit date: ${data.transitDate}`,
    pageWidth / 2, 25, { align: 'center' },
  );

  // ── Transit table ──────────────────────────────────────────────────────────
  const headers = isHi
    ? [['ग्रह', 'वर्तमान राशि', 'भाव', 'मूल', 'वेध', 'स्थिति', 'अंक', 'प्रभाव']]
    : [['Planet', 'Current Sign', 'House', 'Base', 'Vedha', 'Status', '+/-', 'Key Effect']];

  const tableBody = data.results.map(r => [
    `${r.planet.symbol} ${isHi ? r.planet.hi : r.planet.en}`,
    `${RASHIS[r.currentRashi]?.symbol ?? ''} ${isHi ? RASHIS[r.currentRashi]?.hi : RASHIS[r.currentRashi]?.en}`,
    String(r.houseFromMoon),
    r.baseFavorable ? (isHi ? 'शुभ' : 'Fav') : (isHi ? 'अशुभ' : 'Unfav'),
    r.vedhaActive ? (r.vedhaNote ?? 'Yes') : (isHi ? 'नहीं' : 'No'),
    r.effectiveStatus === 'favorable'
      ? (isHi ? 'शुभ ✓' : 'Favorable ✓')
      : r.effectiveStatus === 'mixed'
      ? (isHi ? 'वेध ⚠' : 'Vedha ⚠')
      : (isHi ? 'अशुभ ✗' : 'Unfav ✗'),
    r.scoreContribution > 0 ? '+1' : '0',
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: 22 }, 1: { cellWidth: 26 }, 2: { cellWidth: 13, halign: 'center' },
      3: { cellWidth: 16, halign: 'center' }, 4: { cellWidth: 20 },
      5: { cellWidth: 24, halign: 'center' }, 6: { cellWidth: 12, halign: 'center' },
      7: { cellWidth: 'auto' },
    },
    didParseCell(info) {
      if (info.column.index === 5 && info.section === 'body') {
        const txt = info.cell.text[0] ?? '';
        if (txt.includes('✓')) info.cell.styles.textColor = [22, 163, 74];
        else if (txt.includes('⚠')) info.cell.styles.textColor = [217, 119, 6];
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(
    isHi ? `समग्र स्कोर: ${data.overallScore}/9` : `Overall Score: ${data.overallScore}/9`,
    20, tableEnd,
  );

  // ── Bilingual narrative section ────────────────────────────────────────────
  doc.addPage();
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(isHi ? 'विश्लेषण विवरण' : 'Analysis Narrative', pageWidth / 2, 14, { align: 'center' });

  // English block
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text('English', 14, 24);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  const enLines = doc.splitTextToSize(data.narrativeEn, pageWidth - 28);
  doc.text(enLines, 14, 30);

  const enEnd = 30 + enLines.length * 4 + 8;

  // Hindi block (note: standard helvetica doesn't render Devanagari — romanised fallback)
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text('Hindi (Transliterated)', 14, enEnd);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  // Strip Devanagari as jsPDF cannot render it without font embedding
  const safeHindi = data.narrativeHi.replace(/[\u0900-\u097F]/g, '?');
  const hiLines = doc.splitTextToSize(safeHindi, pageWidth - 28);
  doc.text(hiLines, 14, enEnd + 6);
  ...
  doc.text(
    `Ayanamsa: ${data.ayanamsa ?? 'Lahiri'} | Engine: ${data.ephemerisMode ?? 'SwissEph'} | Generated: ${new Date().toLocaleString('en-IN')} | Vedic Rajkumar`,
