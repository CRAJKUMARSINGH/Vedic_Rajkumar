...
export function calculatePrecisePlanetaryPositions(dateStr: string, timeStr: string) {
  // Default to IST (UTC+5:30) with a neutral geographic location (lat=0, lon=0)
  const jd = dateTimeToJD(dateStr, timeStr, 5.5);
  const ayanamsa = getLahiriAyanamsa(jd);

  // Compute ascendant using latitude 0° and longitude 0° (equatorial Greenwich)
  const ascendantRaw = calcAscendant(jd, 0, 0, ayanamsa);
  const ascendant = {
    ...ascendantRaw,
    sidereal: ascendantRaw.siderealLongitude,
  };
  ...
  const makePlanet = (name: string, trop: number, retro = false) => {
    ...
    return {
      ...
      degrees: degInRashi(sid),
      house: toHouse(sid),
      dignity: getDignity(name, ri),
      retrograde: retro,
      sidereal: sid,
    };
  };
// Duplicate makePlanet block removed;
