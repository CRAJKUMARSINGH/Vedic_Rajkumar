...
export const CompatibilityReportComponent = ({
  ...
  return (
    <div
      className={cn('bg-card border border-border rounded-xl p-6 shadow-sm space-y-6', className)}
    >
      {/* Header */}
      <div className="text-center border-b border-border pb-4">
        <div className="flex items-center justify-center gap-3 mb-2">
          <Heart className="h-6 w-6 text-red-500" />
          <h2 className={cn('text-2xl font-bold text-primary', isHi && 'font-hindi')}>{t.title}</h2>
          <Heart className="h-6 w-6 text-red-500" />
        </div>
        <p className={cn('text-muted-foreground', isHi && 'font-hindi')}>{t.subtitle}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-4 border border-blue-200">
          ...
          <div className="text-sm text-blue-700">{malePartner.dateOfBirth}</div>
        </div>
        ...
        <div className="bg-pink-50 dark:bg-pink-950/20 rounded-lg p-4 border border-pink-200">
          ...
          <div className="text-sm text-pink-700">{femalePartner.dateOfBirth}</div>
        </div>
      </div>

      {/* Overall Score with Circular Visualizer */}
      <div className="bg-muted/30 rounded-xl p-6 border border-primary/5 flex flex-col md:flex-row items-center justify-around gap-6">
        {/* SVG Circular Indicator */}
        <div className="relative flex-shrink-0 animate-in zoom-in-50 duration-500">
          {(() => {
            const radius = 54;
            const circ = 2 * Math.PI * radius;
            const offset = circ * (1 - report.percentage / 100);
            ...
            return (
              <>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span
                    className={cn('text-3xl font-serif font-black leading-none', textColor)}
                    data-testid="match-score"
                  >
                    {report.totalPoints}
                  </span>
                  <span className="text-[10px] text-muted-foreground mt-1 font-mono uppercase tracking-wider">
                    {t.outOf} {report.maxPoints}
                  </span>
                </div>
              </>
            );
        ...
        <div className="text-center md:text-left space-y-2 max-w-sm">
          ...
          <h3 className="font-serif text-2xl font-black text-foreground tracking-wide flex items-center justify-center md:justify-start gap-2">
            {getCompatibilityIcon(report.overallCompatibility)}
            <span
              className={cn(
                'px-3 py-0.5 rounded-lg border text-sm font-semibold',
                getCompatibilityColor(report.overallCompatibility)
              )}
            >
              {isHi
                ? report.overallCompatibility === 'Excellent'
                  ? t.excellent
                  : report.overallCompatibility === 'Good'
                    ? t.good
                    : report.overallCompatibility === 'Average'
                      ? t.average
                      : t.poor
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            {isHi
              ? `आपके 36 गुणों में से ${report.totalPoints} गुण मिलते हैं। यह कुंडली मिलान (${report.percentage.toFixed(1)}%) अनुकूलता को दर्शाता है।`
              : `Your Ashta Koota matching shares ${report.totalPoints} out of 36 gunas, indicating a match compatibility rate of ${report.percentage.toFixed(1)}%.`}
          </p>
        </div>
      </div>
        ...
        <div className="bg-red-50 dark:bg-red-950/20 rounded-lg p-4 border border-red-200">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3
                className={cn(
                  'font-semibold text-red-900 dark:text-red-100 mb-2',
                  isHi && 'font-hindi'
                )}
              >
              ...
              <p
                className={cn('text-sm text-red-800 dark:text-red-200 mb-3', isHi && 'font-hindi')}
              >
                {isHi
              ...
              {report.dosha.remedies.length > 0 && (
                <div>
                  <h4
                    className={cn(
                      'font-medium text-red-900 dark:text-red-100 mb-2',
                      isHi && 'font-hindi'
                    )}
                  >
                  ...
                  <ul className="space-y-1">
                    {report.dosha.remedies.slice(0, 3).map((remedy, index) => (
                      <li
                        key={index}
                        className="flex items-start gap-2 text-xs text-red-800 dark:text-red-200"
                      >
      ...
      <div className="space-y-4">
        <h3 className={cn('text-lg font-bold', isHi && 'font-hindi')}>{t.categories}</h3>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {isHi
                      ? category.compatibility === 'Excellent'
                        ? t.excellent
                        : category.compatibility === 'Good'
                          ? t.good
                          : category.compatibility === 'Average'
                            ? t.average
                            : t.poor
      ...
      {/* Recommendations */}
      <div className="space-y-3">
        <h3 className={cn('text-lg font-bold', isHi && 'font-hindi')}>{t.recommendations}</h3>
