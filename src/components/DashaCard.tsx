...
function AntarDashaRow({
  ...
  return (
    <div
      className={`border rounded-md overflow-hidden ${antar.isActive ? 'border-primary' : 'border-border'}`}
    >
      <button
        onClick={() => setOpen(o => !o)}
        ...
        <div className="px-3 pb-3 pt-1 space-y-2 bg-muted/20">
          ...
          {antar.pratyantardashas && antar.pratyantardashas.length > 0 ? (
            <div className="space-y-1">
              <p
                className={`text-xs font-semibold text-muted-foreground ${isHi ? 'font-hindi' : ''}`}
              >
            ...
            <p className={`text-xs text-muted-foreground italic ${isHi ? 'font-hindi' : ''}`}>
              {isHi
                ? 'प्रत्यंतर्दशा केवल सक्रिय अंतर्दशा के लिए उपलब्ध है।'
                : 'Pratyantardashas available for active antardasha only.'}
            </p>
          )}
        </div>
      )}
...
function MahadashaRow({ maha, isHi }: { maha: DashaPeriod; isHi: boolean }) {
  ...
  return (
    <div
      className={`border-2 rounded-lg overflow-hidden ${colorClass} ${maha.isActive ? 'ring-2 ring-primary ring-offset-1' : ''}`}
    >
      <button
        onClick={() => setOpen(o => !o)}
...
const DashaCard = ({ birthDate, birthTime, lang, accurateMoonLongitude }: DashaCardProps) => {
  const isHi = lang === 'hi';
  const [showAll, setShowAll] = useState(false);

  let dashaResult;
  try {
    dashaResult = calculateVimshottariDasha(birthDate, birthTime, 3, accurateMoonLongitude);
  } catch {
    return null;
  }
  ...
  return (
    <div className="auspicious-card max-w-2xl mx-auto shadow-xl">
      <div className="auspicious-header">
        <div className="flex items-center gap-2">
          <span className="text-2xl">🪐</span>
          <span className={`font-bold tracking-wide ${isHi ? 'font-hindi' : ''}`}>
      ...
      <div className="p-6 space-y-6">
        <p className={`text-sm text-[#8B4513] font-medium italic ${isHi ? 'font-hindi' : ''}`}>
          {isHi
        ...
        <div className="bg-muted/40 rounded-lg p-3 grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p className="font-semibold">
              {balanceYears.toFixed(2)} {isHi ? 'वर्ष' : 'yrs'}
            </p>
          </div>
          {currentMahadasha && (
            <div>
              ...
              <p className="font-semibold">
                {getPlanetSymbol(currentMahadasha.planet)}{' '}
                {planetName(currentMahadasha.planet, isHi)}
              </p>
            </div>
          )}
        </div>
          ...
          <div className="border border-primary/30 rounded-lg p-3 bg-primary/5 space-y-1">
            ...
            <p className={`text-sm ${isHi ? 'font-hindi' : ''}`}>
              <span className="font-semibold">
                {planetName(currentMahadasha.planet, isHi)} {isHi ? 'महादशा' : 'Mahadasha'}
              </span>
              {currentAntardasha && (
                <>
                  {' '}
                  /{' '}
                  <span className="font-semibold">
                    {planetName(currentAntardasha.planet, isHi)} {isHi ? 'अंतर्दशा' : 'Antardasha'}
                  </span>
                </>
              )}
              {currentPratyantardasha && (
                <>
                  ...
                  /{' '}
                  <span className="font-semibold">
                    {planetName(currentPratyantardasha.planet, isHi)}{' '}
                    {isHi ? 'प्रत्यंतर्दशा' : 'Pratyantardasha'}
                  </span>
                </>
              )}
            </p>
            {currentAntardasha && (
              <p className="text-xs text-muted-foreground">
                {isHi ? 'अंतर्दशा समाप्ति:' : 'Antardasha ends:'}{' '}
          ...
          >
            {showAll
              ? isHi
                ...
                : 'Show less'
              : isHi
                ? `सभी ${mahadashas.length} महादशाएं दिखाएं`
                : `Show all ${mahadashas.length} mahadashas`}
          </button>
        )}
