            ...
            {results.map((r, i) => {
              ...
              return (
                <>
                  ...
                  {/* Expanded Life-Area Details + Remedies */}
                  {isExpanded && lifeAreas && (
                    <tr className={`border-t border-border ${i % 2 === 0 ? "bg-card" : "bg-muted/30"}`}>
                      <td colSpan={9} className="px-4 py-3 space-y-3">
                        ...
                        {/* Per-planet remedies for unfavorable transits */}
                        {!r.baseFavorable && PLANET_REMEDIES[r.planet.en] && (
                          <details className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
                            <summary className={`cursor-pointer text-amber-800 dark:text-amber-200 font-semibold text-sm ${isHi ? "font-hindi" : ""}`}>
                              🙏 {isHi ? "उपाय देखें" : "View Remedies"}
                            </summary>
                            <ul className={`list-disc list-inside mt-2 space-y-1 text-xs text-amber-900 dark:text-amber-100 ${isHi ? "font-hindi" : ""}`}>
                              {(isHi ? PLANET_REMEDIES[r.planet.en].hi : PLANET_REMEDIES[r.planet.en].en).map((remedy, idx) => (
