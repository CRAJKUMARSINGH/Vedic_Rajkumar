...
import {
  answerHoraryQuestion,
  getHoraryCategories,
  type HoraryAnswer,
} from '@/services/horaryAstrologyService';
...
const HoraryAstrologyPage = () => {
  ...
  return (
    <>
      <SEO
        title="Horary Astrology - Prashna Kundali"
        description="Ask any question and get answers through Horary Astrology (Prashna Kundali). Get instant astrological guidance for love, career, health and more."
        keywords="horary astrology, prashna kundali, ask question astrology, vedic horary, prashna jyotish"
        canonical="/horary"
      />
      <div className="min-h-screen bg-background">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-xl font-bold ${isHi ? 'font-hindi' : ''}`}>
                  {isHi ? 'प्रश्न कुंडली' : 'Horary Astrology'}
                </h1>
                <p className="text-xs text-muted-foreground">
                  {isHi ? 'प्रश्न पूछें और उत्तर पाएं' : 'Ask a Question • Get Cosmic Answers'}
                </p>
              </div>
            </div>
        ...
        <main className="container max-w-4xl mx-auto px-4 py-8 space-y-6">
          {/* Universal Prasna Research — Prasna Marga integration (top of page) */}
          ...
          <div className="bg-card border rounded-xl p-5 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {isHi
                ? 'जब आप कोई प्रश्न पूछते हैं, उस क्षण की ग्रह स्थिति उत्तर देती है।'
                : 'The planetary positions at the moment you ask a question reveal the answer. Think clearly about your question before asking.'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {categories.map(cat => (
              <button
                key={cat.value}
                onClick={() =>
                  setQuestion(prev => prev || `Will my ${cat.label.toLowerCase()} improve?`)
                }
          ...
          <div className="bg-card border rounded-xl p-5 space-y-3">
            <label className="text-sm font-medium">
              {isHi ? 'अपना प्रश्न लिखें' : 'Type your question'}
            </label>
            <textarea
              value={question}
              ...
              placeholder={
                isHi
                  ? 'उदाहरण: क्या मुझे नई नौकरी मिलेगी?'
                  : 'e.g., Will I get a new job this year? Will my relationship improve?'
              }
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {isHi
                  ? 'प्रश्न स्पष्ट और विशिष्ट होना चाहिए'
                  : 'Be specific and sincere with your question'}
              </p>
              <button
                onClick={handleAsk}
          ...
          {answer && (
            <div className="space-y-4">
              <div
                className={`border-2 rounded-xl p-6 text-center ${ANSWER_STYLES[answer.answer]}`}
              >
