...
const YogasIdentificationDashboard = lazy(() => import('@/components/YogasIdentificationDashboard'));
            ...
            <Tabs defaultValue="chart" className="space-y-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                  ...
                  <button type="button" onClick={() => { setRawBirth(null); setBirthInput(null); }} className="text-sm text-primary underline underline-offset-2">
                    {isHi ? 'बदलें' : 'Change'}
                  </button>
                </div>
              </div>
              ...
              <TabsContent value="extended" className="mt-0">
                <p className="text-xs text-muted-foreground mb-4">
                  {isHi
                    ? 'डेमो डेटा पर आधारित विस्तृत योग पहचान दृश्य।'
                    : 'Extended yoga identification view with demo data.'}
                </p>
                <Suspense fallback={<div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto" />}>
                  <YogasIdentificationDashboard />
                </Suspense>
              </TabsContent>
            </Tabs>
          )}
