...
const AIPredictionsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'predictions' | 'insights' | 'periods' | 'synthesis'
  >('overview');
  ...
  const TYPE_COLORS = {
    ...
    challenge: 'bg-red-50 border-red-200 text-red-800',
  ...
  return (
    <>
      <SEO
        title="AI Astrology Predictions - Personalized Insights"
        description="Get AI-powered personalized astrology predictions for career, love, health, wealth and spiritual growth based on your birth chart."
        keywords="ai astrology, personalized predictions, birth chart analysis, career prediction, love prediction"
        canonical="/ai-predictions"
      />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-xl font-bold ${isHi ? 'font-hindi' : ''}`}>
                  {isHi ? 'AI भविष्यवाणी' : 'AI Predictions'}
                </h1>
                <p className="text-xs text-muted-foreground">
                  {isHi
                    ? 'व्यक्तिगत ज्योतिष अंतर्दृष्टि'
                    : 'Personalized Astrological Intelligence'}
                </p>
              </div>
            </div>
            ...
            <div className="max-w-2xl mx-auto bg-card border rounded-xl p-6 shadow-sm">
              <div className="text-center mb-4">
                ...
                <p className="text-sm text-muted-foreground">
                  {isHi
                    ? 'AI-संचालित व्यक्तिगत ज्योतिष भविष्यवाणी के लिए जन्म विवरण दर्ज करें'
                    : 'Enter birth details for AI-powered personalized astrology predictions'}
                </p>
              </div>
              ...
              {report && (
                <>
                  ...
                  <div className="flex gap-1 bg-muted rounded-lg p-1 overflow-x-auto">
                    {(['overview', 'predictions', 'insights', 'periods', 'synthesis'] as const).map(
                      tab => (
                        <button
                          key={tab}
                          onClick={() => setActiveTab(tab)}
                          className={`flex-1 min-w-[100px] py-1.5 text-xs rounded-md font-medium transition-colors ${activeTab === tab ? 'bg-background shadow text-foreground' : 'text-muted-foreground'}`}
                        >
                          {tab === 'synthesis'
                            ? 'Synthesis Engine ✨'
                      ...
                      <div className="grid grid-cols-5 gap-2">
                        {report.predictions.map(p => (
                          ...
                          >
                            ...
                            <div
                              className={`text-sm font-bold ${p.confidence >= 70 ? 'text-green-600' : p.confidence >= 50 ? 'text-yellow-600' : 'text-red-500'}`}
                            >
                              {p.confidence}%
                            </div>
                          </div>
                      ...
                      {report.predictions.map(p => (
                        <div key={p.category} className="bg-card border rounded-xl p-4">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span
                              className={`ml-auto text-xs px-2 py-0.5 rounded-full ${p.confidence >= 70 ? 'bg-green-100 text-green-800' : p.confidence >= 50 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}
                            >
                  ...
                  {activeTab === 'synthesis' && (
                    <div className="mt-4">
                      <SynthesisDashboard chart={chart} isHi={isHi} />
                    </div>
                  )}
                </>
              )}
