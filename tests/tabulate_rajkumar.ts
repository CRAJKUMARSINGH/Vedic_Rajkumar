...
async function run() {
  const name  = 'Rajkumar';
  const date  = '1963-09-15';
  const time  = '06:00'; // IST
  const lat   = 23.84;
  const lon   = 73.71;
  const place = 'Aspur/Nandli, Rajasthan';
  ...
  console.log(`\n${TOP}`);
  console.log(`║  ${name} — जन्म कुंडली  |  ${date}  ${time} IST  |  ${place.padEnd(20)} ║`);
  console.log(LINE);
  ...
  if (dasha?.sequence) {
    console.log('\nDasha Sequence (Upcoming):');
    dasha.sequence.slice(0,6).forEach((d: any) =>
      console.log(`  ${String(d.planet).padEnd(8)} ${d.startDate} → ${d.endDate}`)
    );
  }
}
